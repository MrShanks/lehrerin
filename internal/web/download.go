...
{{define "day"}}<h1>Daily lesson plan</h1><p class="meta">{{.Data.Date}} · {{.Data.Teacher}}{{if .Data.School}} · {{.Data.School}}{{end}}</p>{{range .Data.Overrides}}{{template "event" .}}{{end}}{{range .Data.Lessons}}{{template "lesson" .}}{{end}}{{if and (not .Data.Overrides) (not .Data.Lessons)}}<p>No lessons scheduled.</p>{{end}}{{end}}
{{define "week"}}<h1>Weekly lesson plan</h1><p class="meta">{{.Data.Start}} – {{.Data.End}} · {{.Data.Teacher}}{{if .Data.School}} · {{.Data.School}}{{end}}</p>{{range .Data.Days}}<section class="week-day"><h2>{{.Date}}</h2>{{range .Overrides}}{{template "event" .}}{{end}}{{range .Lessons}}{{template "lesson" .}}{{end}}{{if and (not .Overrides) (not .Lessons)}}<p>No lessons scheduled.</p>{{end}}</section>{{end}}{{end}}
